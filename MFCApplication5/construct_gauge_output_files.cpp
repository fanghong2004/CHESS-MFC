...
)
{
	...
	for (int gauge_inx = 0; gauge_inx != GAUGE_NUM; gauge_inx++) {
		if (command_line->b != NULL) { //basin level output
			strcpy_s(outGDHfile[gauge_inx], outPutPath);
			strcpy_s(outGDGfile[gauge_inx], outPutPath);
			//strcpy(outBMHfile,outPutPath);
			//strcpy(outBMGfile,outPutPath);
			strcat_s(outGDHfile[gauge_inx], "Gauge_daily_hydro_");
			strcat_s(outGDGfile[gauge_inx], "Gauge_daily_plant_");

			_itoa_s(patch[(gauge_list[gauge_inx])].ID, gauge_name, 10);

			strcat_s(outGDHfile[gauge_inx], gauge_name);
			strcat_s(outGDGfile[gauge_inx], gauge_name);

			strcat_s(outGDHfile[gauge_inx], ".dat");
			strcat_s(outGDGfile[gauge_inx], ".dat");
			...
			if (err = fopen_s(&DM_outfiles->fGaugeDailyHydro[gauge_inx], outGDHfile[gauge_inx], "w")) {
			...
			if (err = fopen_s(&DM_outfiles->fGaugeDailyPlant[gauge_inx], outGDHfile[gauge_inx], "w")) {
