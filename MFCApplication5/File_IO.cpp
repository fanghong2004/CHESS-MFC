...
void input_header(int maxr, int maxc, char *fndem, int arc_flag) {
	...
	if (err = fopen_s(&in1, fndem, "r")) {
		...
		if (arc_flag == 0) {
			for (i = 0; i < 9; i++)
				fscanf_s(in1, "%s", tmp);
			fscanf_s(in1, "%d %*s %d", &maxr, &maxc);
		}
		else
			fscanf_s(in1, "%*s %d %*s %d", &maxc, &maxr);
		fclose(in1);
	}
	return;
}
...
void header_help(int maxr, int maxc, char *fnhdr) {
	...
	scanf_s("%d %d", &maxr, &maxc);

	//if ((in1 = fopen(fnhdr, "w")) == NULL) {
	if (err = fopen_s(&in1, fnhdr, "r")) {
...
void	read_geo_images(struct patch_object *patch, struct command_line_object *command_line, int rows, int cols, double cellsize, double xll, double yll,
	...
	// copy the root filename into the specific filenamesfscanf
	strcat_s(filename,sizeof(filename)+100, prefix);
	strcpy_s(fnpatch, filename);
	strcpy_s(fndem, filename);
	strcpy_s(fnslope, filename);
	strcpy_s(fnaspect, filename);
	strcpy_s(fneast_horizon, filename);
	strcpy_s(fnwest_horizon, filename);
	strcpy_s(fnsoil, filename);
	strcpy_s(fnveg, filename);
	strcpy_s(fnroads, filename);

	//xu.
	strcpy_s(fnstreamorder, filename);
	strcpy_s(fnbasins, filename);
	strcpy_s(fngauges, filename);
	strcpy_s(fnclimate, filename);
	strcpy_s(fnlatitude, filename);
	strcpy_s(fnreservoir, filename);

	// append '.' extensions to each filename (these should be generalized) */
	strcat_s(fnpatch, ".patch");
	strcat_s(fndem, ".dem");
	strcat_s(fnslope, ".slope");
	strcat_s(fnaspect, ".aspect");
	strcat_s(fneast_horizon, ".east");
	strcat_s(fnwest_horizon, ".west");
	strcat_s(fnsoil, ".soil");
	strcat_s(fnveg, ".veg");
	strcat_s(fnroads, ".road");


	//xu.
	strcat_s(fnstreamorder, ".streamorder");
	strcat_s(fnbasins, ".basin");

	char char_thread[10];
	_itoa_s(thread_num, char_thread, 10);
	strcat_s(fnbasins, char_thread);
	cout << fnbasins << endl;

	strcat_s(fngauges, ".gauge");
	strcat_s(fnclimate, ".climate");
	strcat_s(fnlatitude, ".latitude");
	strcat_s(fnreservoir, ".reservoir");
