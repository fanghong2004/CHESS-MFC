...
)
{
	...
	if (out_patch_flag == 1 && command_line->p != NULL) {
		strcpy_s(outPDHfile, outPutPath);
		strcpy_s(outPDGfile, outPutPath);
		//strcpy_s(outPMHfile,outPutPath);
		//strcpy_s(outPMGfile,outPutPath);
		_itoa_s(current_date.year, buffer, 10);

		strcat_s(outPDHfile, buffer);
		strcat_s(outPDGfile, buffer);
		//strcat_s(outPMHfile,buffer);
		//strcat_s(outPMGfile,buffer);

		strcat_s(outPDHfile, "_");
		strcat_s(outPDGfile, "_");
		//strcat_s(outPMHfile,"_");
		//strcat_s(outPMGfile,"_");

		_itoa_s(current_date.month, buffer, 10);
		...
		strcat_s(outPDHfile, "_daily_hydro.dat");
		strcat_s(outPDGfile, "_daily_plant.dat");
		//strcat_s(outPMHfile,"_monthly_hydro.dat");
		//strcat_s(outPMGfile,"_monthly_Plant.dat");
		...
		if (err = fopen_s(&DM_outfiles->fPatchDailyHydro, outPDHfile, "w")) {
		...
		if (err = fopen_s(&DM_outfiles->fPatchDailyPlant, outPDHfile, "w")) {
