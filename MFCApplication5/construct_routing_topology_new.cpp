...

{
	...
	/*--------------------------------------------------------------*/
	strcat_s(routing_filename,sizeof(routing_filename)+100,FlowTableName);
	//if ((routing_file = fopen(routing_filename, "r")) == NULL) {
	if (err = fopen_s(&routing_file, routing_filename, "r")) {
		fprintf(stderr, "FATAL ERROR:  Cannot open routing file %s\n",
			routing_filename);
		exit(0);
	} /*end if*/
	fscanf_s(routing_file, "%d", &num_patches);
	...
	for (i = 0; i < num_patches; i++) {
		fscanf_s(routing_file, "%d %lf %lf %lf %lf %d %lf %d",
			&patch[i].ID,
