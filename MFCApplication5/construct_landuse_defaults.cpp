	...
	struct landuse_default *landuse_default_object_list)
{
	...
	for (i = 0; i < nytpes; i++) {
		...
		fscanf_s(default_file, "%d", &(landuse_default_object_list[i].ID));
		read_record(default_file, record);
		fscanf_s(default_file, "%lf", &(landuse_default_object_list[i].irrigation));
		read_record(default_file, record);
		landuse_default_object_list[i].irrigation /= 365;
		fscanf_s(default_file, "%lf", &(landuse_default_object_list[i].fertilizer_NO3));
		read_record(default_file, record);
		landuse_default_object_list[i].fertilizer_NO3 /= 365;
		fscanf_s(default_file, "%lf", &(landuse_default_object_list[i].fertilizer_NH4));
		read_record(default_file, record);
		landuse_default_object_list[i].fertilizer_NH4 /= 365;
		fscanf_s(default_file, "%lf", &(landuse_default_object_list[i].septic_NO3_load));
		read_record(default_file, record);
		landuse_default_object_list[i].septic_NO3_load /= 365;
		fscanf_s(default_file, "%lf", &(landuse_default_object_list[i].septic_water_load));
		read_record(default_file, record);
		landuse_default_object_list[i].septic_water_load /= 365;
		fscanf_s(default_file, "%lf", &(landuse_default_object_list[i].detention_store_size));
		read_record(default_file, record);
