...
void construct_patch(struct patch_object * patch, struct command_line_object * command_line, int rows, int cols, char *filename, char *prefix, int num_patches)
{
	...
	strcpy_s(filename_re, filename);
	strcat_s(filename,sizeof(filename)+100, prefix);
	strcpy_s(fnclimatedef, filename);
	strcpy_s(fnsoildef, filename);
	strcpy_s(fnvegdef, filename);
	strcpy_s(fnlandusedef, filename);
	strcpy_s(fnchanneldef, filename);
	strcpy_s(fnreservoirdef, filename);

	// append '.' extensions to each filename (these should be generalized) 
	strcat_s(fnsoildef, "_soil.def");
	strcat_s(fnvegdef, "_veg.def");
	strcat_s(fnclimatedef, "_climate.def");
	strcat_s(fnlandusedef, "_landuse.def");
	strcat_s(fnchanneldef, "_channel.def");
	strcat_s(fnreservoirdef, "_reservoir.def");

	//===========================================================================================================================
	// 1. reading soil default files
	//===========================================================================================================================
	//if ((default_file = fopen(fnsoildef, "r")) == NULL) {
	if ((err = fopen_s(&default_file,fnsoildef, "r"))) {
		fprintf(stderr, "cannot open soil default file %s\n", fnsoildef);
		return;
	}
	else {
		fscanf_s(default_file, "%d ", &soiltypes);
		read_record(default_file, record);
		fscanf_s(default_file, "%d ", &nvars);
		read_record(default_file, record);
		printf("\n");
	...
	if ((err = fopen_s(&default_file, fnvegdef, "r"))) {
		fprintf(stderr, "cannot open vegetation default file %s\n", fnvegdef);
		return;
	}
	else {
		fscanf_s(default_file, "%d ", &vegtypes);
		read_record(default_file, record);
		fscanf_s(default_file, "%d ", &nvars);
		read_record(default_file, record);
		printf("\n");
	...
	if ((err = fopen_s(&default_file, fnlandusedef, "r"))) {
		fprintf(stderr, "cannot open soil default file %s\n", fnlandusedef);
		return;
	}
	else {
		fscanf_s(default_file, "%d ", &landusetypes);
		read_record(default_file, record);
		fscanf_s(default_file, "%d ", &nvars);
		read_record(default_file, record);
		printf("\n");
	...
	if ((err = fopen_s(&default_file, fnclimatedef, "r"))) {
		fprintf(stderr, "cannot open climate default file %s\n", fnclimatedef);
		return;
	}
	else {
		fscanf_s(default_file, "%d ", &climatetypes);
		read_record(default_file, record);
		fscanf_s(default_file, "%d ", &nvars);
		read_record(default_file, record);
		printf("\n", climatetypes);
	...
	if (command_line->cf == TRUE) {
		...
		if ((err = fopen_s(&default_file, fnchanneldef, "r"))) {
			fprintf(stderr, "cannot open channel default file %s\n", fnchanneldef);
			return;
		}
		else {
			fscanf_s(default_file, "%d ", &channeltypes);
			read_record(default_file, record);
			fscanf_s(default_file, "%d ", &nvars);
			read_record(default_file, record);
			printf("\n");
	...
	if (command_line->cf == TRUE) {
		...
		if ((err = fopen_s(&default_file, fnreservoirdef, "r"))) {
			fprintf(stderr, "cannot open reservoir default file %s\n", fnreservoirdef);
			return;
		}
		else {
			fscanf_s(default_file, "%d ", &reservoirtypes);
			read_record(default_file, record);
			fscanf_s(default_file, "%d ", &nvars);
			read_record(default_file, record);
			printf("\n");
