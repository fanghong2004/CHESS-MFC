	...
	double DailyRain[][12][31], double DailyTmax[][12][31], double DailyTmin[][12][31], double AnnCO2[])
{
	...
	for (year = InRange.start_year; year <= InRange.end_year; year++) {
		...
		for (month = InRange.start_month; month <= InRange.end_month; month++) {
			...
			for (day = InRange.start_day; day <= InRange.end_day; day++) {
				...
				fscanf_s(inClimFiles.pPrec, "%d %d %d %lf ", &yr, &mo, &dy, &DailyRain[year - InRange.start_year][month - InRange.start_month][day - InRange.start_day]);
				...
				fscanf_s(inClimFiles.pTmax, "%d %d %d %lf ", &yr, &mo, &dy, &DailyTmax[year - InRange.start_year][month - InRange.start_month][day - InRange.start_day]);
				if (current_date.year != yr || current_date.month != mo || current_date.day != dy) {
				...
				fscanf_s(inClimFiles.pTmin, "%d %d %d %lf ", &yr, &mo, &dy, &DailyTmin[year - InRange.start_year][month - InRange.start_month][day - InRange.start_day]);
				if (current_date.year != yr || current_date.month != mo || current_date.day != dy) {
	...
	for (year = InRange.start_year; year <= InRange.end_year; year++) {
		...
		fscanf_s(inClimFiles.pCO2, "%d %lf ", &yr, &AnnCO2[year - InRange.start_year]);
