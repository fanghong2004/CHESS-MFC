...
)
{
	...
	fscanf_s(inClimFiles.pPrec, "%d %d %d", &yr, &mo, &dy);
	for (int clim_inx = 0; clim_inx != climate_num; clim_inx++) {
		fscanf_s(inClimFiles.pPrec, "%lf", &daily_clim[clim_inx].rain);
	}
	...
	fscanf_s(inClimFiles.pTmax, "%d %d %d", &yr, &mo, &dy);
	for (int basin_inx = 0; basin_inx != climate_num; basin_inx++) {
		fscanf_s(inClimFiles.pTmax, "%lf", &daily_clim[basin_inx].tmax);
	}
	if (current_date.year != yr || current_date.month != mo || current_date.day != dy) {
	...
	fscanf_s(inClimFiles.pTmin, "%d %d %d", &yr, &mo, &dy);
	for (int clim_inx = 0; clim_inx != climate_num; clim_inx++) {
		fscanf_s(inClimFiles.pTmin, "%lf", &daily_clim[clim_inx].tmin);
	}
	if (current_date.year != yr || current_date.month != mo || current_date.day != dy) {
	...
	if (yr == current_date.year && *CO2_flag == 1) {
		...
		fscanf_s(inClimFiles.pCO2, "%d %lf ", &yr, &(daily_clim->CO2));
		for (int clim_inx = 0; clim_inx != climate_num; clim_inx++) {
