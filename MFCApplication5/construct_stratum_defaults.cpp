	...
	struct stratum_default *stratum_default_object_list)
{
	...
	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%d", &(stratum_default_object_list[i].ID));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++) {
		fscanf_s(default_file, "%s", stmp,100);
		stratum_default_object_list[i].epc.veg_type = parse_veg_type(stmp);
	}
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].K_absorptance));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].K_reflectance));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++) {
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].K_transmittance));
	...
	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].PAR_absorptance));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].PAR_reflectance));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++) {
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].PAR_transmittance));
	...
	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.ext_coef));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].specific_rain_capacity));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].specific_snow_capacity));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].wind_attenuation_coeff));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].ustar_overu));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].mrc.q10));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].mrc.per_N));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.gr_perc));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].lai_stomatal_fraction));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.flnr));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.ppfd_coef));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.topt));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.tmax));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.tcoef));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.psi_open));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.psi_close));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.vpd_open));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.vpd_close));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.gl_smax));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.gl_c));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].gsurf_slope));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].gsurf_intercept));
	read_record(default_file, record);

	//for (i = 0; i<ntypes; i++){
	//	fscanf_s(default_file, "%s", stmp);
	...
	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.max_lai));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.proj_sla));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.lai_ratio));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.proj_swa));
	read_record(default_file, record);

	//for (i = 0; i<ntypes; i++)
	//	fscanf_s(default_file, "%d", &(stratum_default_object_list[i].epc.day_leafon));
	//read_record(default_file, record);

	//for (i = 0; i<ntypes; i++)
	//	fscanf_s(default_file, "%d", &(stratum_default_object_list[i].epc.day_leafoff));
	//read_record(default_file, record);

	//for (i = 0; i<ntypes; i++)
	//	fscanf_s(default_file, "%d", &(stratum_default_object_list[i].epc.ndays_expand));
	//read_record(default_file, record);

	//for (i = 0; i<ntypes; i++)
	//	fscanf_s(default_file, "%d", &(stratum_default_object_list[i].epc.ndays_litfall));
	//read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.leaf_cn));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.leaflitr_cn));
	read_record(default_file, record);
	...
	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.froot_cn));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.livewood_cn));
	read_record(default_file, record);


	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].min_heat_capacity));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].max_heat_capacity));
	read_record(default_file, record);
	...
	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.storage_transfer_prop));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.leaf_turnover));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.froot_turnover));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++) {
		if ((stratum_default_object_list[i].epc.veg_type == GRASS) || (stratum_default_object_list[i].epc.veg_type == C4GRASS)) {
			fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.deadleaf_turnover));
		}
		else {
			fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.livewood_turnover));
		}
	}
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.daily_fire_turnover));
	read_record(default_file, record);


	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.kfrag_base));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++) {
		fscanf_s(default_file, "%lf", &(ftmp));
	...
	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.leaflitr_flab));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.leaflitr_fscel));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.leaflitr_flig));
	read_record(default_file, record);
	...
	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.frootlitr_flab));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.frootlitr_fscel));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.frootlitr_flig));
	read_record(default_file, record);
	...
	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.deadwood_fscel));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.deadwood_flig));
	read_record(default_file, record);
	...
	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.alloc_frootc_leafc));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.alloc_crootc_stemc));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.alloc_stemc_leafc));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.alloc_livewoodc_woodc));
	read_record(default_file, record);
	...
	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.alloc_prop_day_growth));
	read_record(default_file, record);


	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.alloc_maxlgf));
	read_record(default_file, record);



	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.height_to_stem_exp));
	read_record(default_file, record);

	for (i = 0; i < ntypes; i++)
		fscanf_s(default_file, "%lf", &(stratum_default_object_list[i].epc.height_to_stem_coef));
	read_record(default_file, record);
	read_record(default_file, record);
